...
int main(int argc, char *argv[])
{
	...
	glutAddMenuEntry("No Advance", 9997);
...
void My_Display() {
	...
	if (!isMissionCompleted) {
		...
		if (marco.isDieing) {
			...
			if (fboChoice != FBOChoice::MARCO_DEATH) {
				lastFboChoice = fboChoice;
				fboChoice = FBOChoice::MARCO_DEATH;
			...
			if (marco.state == State::DEATH_WHICH_ATTACK_FROM_LEFT && !marco.isDead) {
				marco.deathLeft();
				marco.isDead = marco.nextFrame(10);
			}
			else if (marco.state == State::DEATH_WHICH_ATTACK_FROM_RIGHT && !marco.isDead) {
				marco.deathRight();
				marco.isDead = marco.nextFrame(10);
			}

			if (marco.isDead && fboManager->isAnimationEnd()) {
				marco.reborn();

				mAttack.clear();
				eAttack.clear();

				fboChoice = (lastFboChoice == FBOChoice::MARCO_DEATH) ? FBOChoice::NONE : lastFboChoice;
				fboChoiceMenuEvents(fboChoice);
			}
		}
